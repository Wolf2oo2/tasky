  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      child:Text(
            ...
            Expanded(
              child: Padding(padding: EdgeInsetsDirectional.symmetric(horizontal: 16),
                    ...
                    onTap: (bool? val, int? index) async {
                      ...
                      if (allData != null) {
                        ...
                        (jsonDecode(allData) as List)
                            .map(
                              (element) =>
                              TasksModel.fromJson(element),
                        ).toList();
                        final newIndex = allDataList.indexWhere((e) =>
                        e.id == tasks[index!].id);
                        allDataList[newIndex] = tasks[index!];
                        final encodeData = allDataList
                            .map((e) => e.toMap())
                            .toList();
                        await PreferencesManager().setString(
                          "tasks",
                          jsonEncode(encodeData),
                        );
                        _loadTasks();
                      }
                    }, onDelete: (int? id ) { return _deleteTasks(id);}, onEdit: ()=> _loadTasks(),
                  ),
              ),
            ),
